...
This updated version of the package now also include a multilabel implementation of the algorithm.
...
\section{ CRIS Multilabel }

The package is now updated in order to also aso a multilabel assignment of the CRIS classes. This can be applied by
